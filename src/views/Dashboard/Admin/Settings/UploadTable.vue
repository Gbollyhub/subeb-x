<template>
  ...
  <tr v-for="(item, index) in reports" :key="index" >
    ...
    <td><div class="table-data">{{item.year}}</div></td>
      ...
      <div  @click="deleteActionPlan(item, index)" style="cursor:pointer;padding:10px" class="app-btn danger">Delete</div>
      </div>
              </td>
  </tr>
            </tbody>
...
try {
     await axios.delete(`${url}/remove-allocation?year=${item.year}`,{ headers: { 'Authorization': `Bearer ${token}` } }, { timeout: 10 })
    await axios.delete(`${url}/remove-breakdown?year=${item.year}`,{ headers: { 'Authorization': `Bearer ${token}` } }, { timeout: 10 })
     await axios.delete(`${url}/remove-allreport?year=${item.year}`,{ headers: { 'Authorization': `Bearer ${token}` } }, { timeout: 10 })
     const user_id = JSON.parse(localStorage.getItem('user'))._id;
      const logData = {
           "activities": { activity:  `Action Plan Deleted. Details: Year: ${item.year}`},
            "user_id": user_id,
        }
